...
export default function Header() {
  ...
  return (
    <>
      <nav className="header bg-gray-800 relative">
        <div className="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8">
          <div className="relative flex h-16 items-center justify-between">
            <div className="absolute inset-y-0 left-0 flex items-center sm:hidden">
              {/* Mobile menu button */}
              <button type="button"
                className="relative inline-flex items-center justify-center rounded-md p-2 text-gray-400 hover:bg-gray-700 hover:text-white focus:ring-2 focus:ring-white focus:outline-hidden focus:ring-inset cursor-pointer"
              ...
              <div className="relative ml-3">
                <div>
                  ...
                  >
                    ...
                    {images.length > 0 && <Image className="rounded-full" src={images[0].url} alt={display_name} width={32}  height={32} />}
                  </button>
                </div>
                <div className={`user-menu absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 ring-1 shadow-lg ring-black/5 focus:outline-hidden${userMenuActive ? ' block' : ' hidden'}`} role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button" tabIndex={-1}>
                  <div className='flex flex-col'>
                    <span className="block px-4 py-2 text-sm text-gray-700" role="menuitem">Logged in as: {display_name}</span>
                    <span className="block px-4 py-2 text-sm text-gray-700" role="menuitem">Email: {email}</span>
                    <a href={href} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" role="menuitem" target='_blank'>Your Profile</a>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className={`header-nav bg-gray-800 absolute top-full w-full height-full ${mobileMenuActive ? 'block' : 'hidden'} sm:hidden`} id="mobile-menu">
